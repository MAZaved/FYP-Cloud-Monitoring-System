                ...
                List<ServiceMonitoring> list = service.getListOfServices();
                 ...
                 %>
					  ...
					  <div class="modal-dialog">
						...
						<input  name="page" id="focusedInput" type="hidden" value="monitor_management.jsp">
					...
					<!-- END OF MODAL ServiceModal-->
	                 ...
	                 <div id="monitor-toggle" class="row">
						...
						<div  class="col-md-2">
						  ...
						  <div id="progressType<%=serviceNum%>"class="progress progress-striped">
						  <div id="progress<%=serviceNum%>" class="prog progress-bar progress-bar-danger" style="width: 100%"></div>
						</div>
					...
					$('#startBtn'+<%=serviceNum%>).click(function () {
					     ...
					     $("#progress"+<%=serviceNum%>).removeClass('progress-bar-danger');
					    $("#progress"+<%=serviceNum%>).addClass('progress-bar-success');
					    $("#progressType"+<%=serviceNum%>).addClass('active');
					});
					
					$('#stopBtn'+<%=serviceNum%>).click(function () {
					    
					     $("#progressType"+<%=serviceNum%>).removeClass('progress-bar-success');
					    $("#progress"+<%=serviceNum%>).addClass('progress-bar-danger');
					   
					    $("#progressType"+<%=serviceNum%>).removeClass('active');
					});
					</script>
					</form>
	              <hr>
					...
					$('#stopResourceBtn'+<%=resourceNum%>).click(function () {
		               ...
		               <label style="margin-bottom: 3%;font-size:120%; color: rgb(55, 90, 127)">Agents:</label>
	          			...
	          			<%
							...
							//present all resource agents for each resource monitor
	              ...
	              <hr>
	               ...
	               <label style="margin-bottom: 3%;font-size:150%; color: rgb(55, 90, 127)">Proxy Settings:</label>
                     ...
                     for (ProxySettings proxysettings : proxyList) {
                    	 proxyNum++;
                    	 if(user.getUserId() == proxysettings.getUserId()){           		 
                 %>
