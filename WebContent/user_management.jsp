        ...
        <header class="jumbotron hero-spacer">
        		...
        		<%
                 ...
                 %>
                 	...
                 	<!-- Modal -->
		<div id="userModal<%=loginNum%>" class="modal fade" role="dialog">
		...
		<form data-toggle="validator" action="RegisterServlet" method="POST">
			...
			<div class="modal-content">
			  ...
			  <div class="modal-body">
				...
				<input  name="userId" id="focusedInput" type="hidden" value="<%=login.getUserId()%>">
				
				<div class="form-group">
					<div class="row" style="margin-top: 6%;">
						<div class="form-group col-md-3">
						  <label class="control-label" for="focusedInput">User Name</label>
						  <input name="username" class="form-control input-sm" id="focusedInput" type="text" value="<%=login.getUsername()%>" required>
						  <div class="help-block with-errors"></div>
						</div>
						
						<div class="form-group col-md-2">
						  <label class="control-label" for="focusedInput">Password</label>
						  <input name="password" class="form-control input-sm" id="password<%=loginNum%>" type="password" value="<%=login.getPassword()%>" required>
						  <div class="help-block with-errors"></div>
						</div>
						
						<div class="form-group col-md-4">
						  <label class="control-label" for="focusedInput">Password Confirmation</label>
						  <input  class="form-control input-sm"  data-match="#password<%=loginNum%>" data-match-error="Passwords Don't Match" type="password" value="<%=login.getPassword()%>" required>
						  <div class="help-block with-errors"></div>
						</div>
						
						<div class="form-group col-md-2">
						  <label class="control-label" for="focusedInput">Email</label>
						  <input name="email" class="form-control input-sm" id="focusedInput" type="email" value="<%=login.getEmail()%>" required>
						  <div class="help-block with-errors"></div>
						</div>
					...
					<div class="col-md-3">
						...
						<label for="sel1">User Type:</label>
					      ...
					      <select name="userType" class="form-control" id="sel1">
					        <option><%=login.getUserType()%></option>
					...
					<button name="removeUser" style="margin-top: 5%" class="btn btn-info">Remove User</button>
					</div>
			  ...
			  <div class="modal-footer">
				...
				<button name="updateUser" class="btn btn-primary" type="submit">Save</button>
			  </div>
			</div>
		</form>	
		  </div>
			...
			<div class="col-md-2">
			  ...
			  <p style="font-size:120%"><%=login.getUsername()%></p>
			</div>
			
			<div class="col-md-2">
			  <label class="control-label" for="focusedInput">User Type</label>
			  
				<p style="font-size: 90%"><%=login.getUserType()%></p>
			</div>
			
			
			<div class="col-md-1">
			 <button type="button" class="btn btn-info btn-md" data-toggle="modal" data-target="#userModal<%=loginNum%>">Edit</button>
			</div>
		</div>
